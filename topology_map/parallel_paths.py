...
BATCH_SIZE    = 10
TARGET_SAMPLE_COUNT = 20  # counts ATTEMPTS (same semantics as your old script)
MAX_WORKERS   = 8  # use most cores
MAP_CHUNKSZ   = 2048   # bigger task chunks -> fewer IPC calls
CAND_MULTIPLIER = 4    # oversample factor for dedup/self-filter

OUTPUT_FILE       = ("sampled_path_lengths_LE_250k_pops_w_novalidity2.csv" if USE_WEIGHTED
...
def compute_path_length(pair):
    ...
    src, dst = pair
    try:
        path = _GRAPH.find_path(src, dst, emissions = False)
        if path:
